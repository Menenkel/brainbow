...
import { ExternalLink, Users, Presentation, Leaf, Clock, Calendar, ToggleLeft, ToggleRight } from "lucide-react";
...
export function TodaySchedule() {
  ...
  // Filter events to only show current day
  ...
  // Filter events for upcoming days and group by date
  const now = new Date();
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
      ...
      {upcomingEvents.length === 0 ? (
        ...
        <div className="space-y-6">
          {Object.entries(eventsByDate).slice(0, 7).map(([dateKey, dayEvents]) => {
          ...
          {Object.keys(eventsByDate).length > 7 && (
            <div className="text-center pt-4">
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Showing next 7 days • More events available
              </p>
            </div>
          )}
        </div>
